    ...
    override fun decode(s: String, response: Response): Exception {
        try {
            response.body().asReader(StandardCharsets.UTF_8).use { input ->
                ...
                val dto: JsonObject = JsonObject::class.fromGson(json.toString())
                val error = dto.get("error")
                if (error is JsonPrimitive && error.isString) {
                    val resultDto: AzureFailedResultDto = AzureFailedResultDto::class
                        .fromGson(json.toString())
                ...
                error as JsonObject
                return AzureDecodeException(
                    response.status(), error.get("code").asString,
                    error.get("message").asString, response.request()
                )
            }
